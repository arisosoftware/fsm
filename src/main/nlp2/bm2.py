...
def download_html(url, downloadPageToFile):
    try:
        ...
        if response is not None:
                ...
                with open(downloadPageToFile, "wb") as fsave:
                        fsave.write(html_content)
                return downloadPageToFile
        else:
                return f"N{downloadPageToFile}" 
        # html_content = response.read().decode("utf-8")
        # return html_content
    except Exception as e:
        print(f"Failed to download {url}: {str(e)}")
        return f"X{downloadPageToFile}" 
    ...
    if not a_tags:
        ...
        for a_tag in a_tags:
            ...
            if url:
                ...
                if not os.path.exists(downloadPage) or :
